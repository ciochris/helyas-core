...
def group_chat(session_id):
    try:
        ...
        # ── PROPOSE MEMORY ────────────────────────────────────────────────────
            ...
            memory_prompt = (
                "Genera una proposta di memoria per il progetto Helyas.\n\n"
                "FONTI DISPONIBILI\n\n"
                "1. MEMORIA GLOBALE ESISTENTE — NON RICOPIARE\n"
                "Contiene informazioni già salvate. Serve solo come\n"
                "riferimento per capire il contesto ed evitare duplicati.\n"
                "NON copiare voci da questa sezione nella nuova proposta.\n"
                "NON trattare queste informazioni come decisioni del\n"
                "dibattito corrente.\n\n"
                f"{global_mem.strip() if global_mem else '(nessuna memoria precedente)'}\n\n"
                "2. DIBATTITO CORRENTE — FONTE DELLE SOLE NOVITÀ\n"
                "Questa è l'unica fonte da cui estrarre nuove decisioni.\n\n"
                f"{history_text[:4000]}\n\n"
                "COMPITO\n"
                "Genera una proposta con SOLO informazioni nuove o\n"
                "modificate rispetto alla memoria globale esistente.\n"
                "- Se una decisione è già presente e non è cambiata,\n"
                "  non includerla.\n"
                "- Se il dibattito conferma una decisione già nota\n"
                "  senza cambiarla, non salvarla di nuovo.\n"
                "- Se il dibattito aggiorna una decisione esistente,\n"
                "  salva solo l'aggiornamento specifico.\n"
                "- La sezione BUG TECNICI RISOLTI contiene SOLO bug\n"
                "  tecnici del sistema Helyas (codice, UI, DB, prompt).\n"
                "  NON include problemi operativi del dibattito.\n"
                "- Se non ci sono novità reali, rispondi:\n"
                "  'Nessuna nuova memoria da salvare. Il dibattito ha\n"
                "  usato informazioni già presenti o prodotto solo\n"
                "  chiarimenti temporanei.'\n\n"
                "CONTROLLO ANTI-DUPLICAZIONE\n"
                "Prima di ogni punto chiediti:\n"
                "- Nasce dal dibattito corrente?\n"
                "- È nuovo rispetto alla memoria globale?\n"
                "- Cambia una decisione, regola, bug o prossimo passo?\n"
                "Se la risposta è no, non inserirlo.\n\n"
                "Formato:\n"
